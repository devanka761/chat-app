...
import { IAuthUserB } from "../../interfaces/binder.interfaces"
import { ValidProviders } from "../../types/binder.types"
import { UserProcess } from "../../interfaces/dbserver.interfaces"
import { IPayLoadB, IRepTempB } from "../../interfaces/validate.interfaces"
import validate from "../main/validate"
import {} from "./account.controller"
...
export function authLogin(s: IPayLoadB): IRepTempB {
  if (!s.email || !validate(["email"], s)) return { code: 400 }
...
export function authVerify(s: IPayLoadB): IRepTempB {
  if (!s.email || !validate(["email", "code"], s)) return { code: 404 }
  s.email = s.email.toString().toLowerCase()
  const mailValid = /^[\w-.]+@([\w-]+\.)+[\w-]{2,4}$/g
...
export function processUser(email: string, dbkey: string): IRepTempB {
  const provider: ValidProviders = "mondin"
  ...
  data.user.id = ukey
  delete db.ref.t[dbkey]
  return { code: 200, data: { user: { id: data.user.id, data: data.user.data } } }
}

export function processThirdParty(s: { user: IPayLoadB; provider: string }): IRepTempB {
  const udb = db.ref.u
  const userInfo: IAuthUserB = {
    email: <string>s.user.email,
    id: <string>s.user.id,
    provider: <ValidProviders>s.provider
  }
  const data: { user: { id?: string; data: IAuthUserB } } = { user: { data: userInfo } }
