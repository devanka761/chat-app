...
import { genhex, isProd, normalizeMessage, peerKey } from "../main/helper"
import cfg from "../main/cfg"
import { DatabasesF, IChatsF, IMeF, PeerDB } from "../../interfaces/dbclient.interfaces"
...
export function getMe(uid: string): IRepTempB {
  ...
  const meChatList: IChatsF[] = Object.keys(cdb)
    .filter((k) => {
      return cdb[k].c && cdb[k].c.length >= 1 && cdb[k].u.find((usr) => usr === uid)
    })
    .map((k) => {
      const chatFile = db.fileGet(cdb[k].c?.[0] as string, "user") || {}
      return {
        id: k,
        u: [getUser(uid, cdb[k].u.find((usr) => usr !== uid) as string)],
...
export function setImage(uid: string, s: { img: string; name: string }): IRepTempB {
