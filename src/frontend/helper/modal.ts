import { ModalAlert, ModalConfirm, ModalPrompt, ModalSelect } from "../../interfaces/helper.interfaces"
import { IRepB } from "../../interfaces/validate.interfaces"
...
const modal = {
  ...
  async alert(options: object | string): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      const btn = qulement(".act .btn-ok", el)
      if (s.okx) btn.innerText = s.okx

      qulement(".app").append(el)
      btn.focus()
  ...
  confirm(options: object | string): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      if (s.img) {
        const img = new Image()
        img.src = s.img
        qulement(".box .inf", el).append(img)
        img.onerror = async () => {
          el.classList.add("out")
          await this.waittime()
          el.remove()
      ...
      const btnOk = qulement(".acts .btn-ok", el)
      if (s.okx) btnOk.innerText = s.okx
      const btnCancel = qulement(".acts .btn-cancel", el)
  ...
  select(options: object): Promise<string | null> {
    return new Promise((resolve) => {
      ...
      const form = qulement(".box .inf #modal-radio-form", el) as HTMLFormElement
